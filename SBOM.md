...
프로젝트: 수문장(sumunjang) v0.2.0 · 라이선스: Apache-2.0
기준일: 2026-08-08 · 생성 근거: `uv pip tree`, 패키지 메타데이터

v0.1.0 대비 의존성 변동 없음. 런타임 직접 의존성은 여전히 httpx·uvicorn 둘뿐이며,
0.2.0 에서 추가된 OpenAI 호환 경로(`openai.py`)와 본문 마스킹(`body.py`)도
표준 라이브러리만 쓴다.

## 설계 원칙: 개인정보 경로에 서드파티 코드를 두지 않는다

개인정보를 실제로 읽고 가리는 모듈(`detect`, `mask`, `body`, `anthropic`, `openai`,
`goldenset`, `cli`)은
...
본 v0.2.0은 **AI 모델을 탑재하지 않는다**. 탐지는 전부 규칙과 검증식(체크섬)으로 이루어지며,
모델 가중치·학습 데이터를 포함하지 않는다. 따라서 모델 라이선스 전파 문제가 발생하지 않는다.

한국어 개체명 인식 모델을 얹어 이름·주소를 잡는 방향은 **검토했으나 채택하지 않았다.**
탐지율은 오르지만 이 프로젝트의 두 기둥이 무너진다 — *개인정보가 지나가는 경로에 서드파티
코드를 두지 않는다*(위 설계 원칙)와 *같은 입력이면 언제나 같은 출력이다*. 규칙으로 못 잡는
것을 `goldenset-gaps` 에 0점으로 공표하는 편이, 모델을 얹어 점수를 올리고 그 대신 "왜
이번엔 못 잡았는지" 를 설명할 수 없게 되는 것보다 낫다고 판단했다. 이 문서가 "모델 미탑재"
를 선언하는 이상, 로드맵에 그 반대를 적어 두면 선언의 유효기간을 스스로 흐린다.
