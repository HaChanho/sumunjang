...
def test_겹쳐서_병합된_구간도_정규형으로_동일성을_판단한다():
    """병합 구간은 어떤 개별 탐지와도 좌표가 달라 정규형 조회가 빈다.

    그러면 원문 조각이 세션 키가 되어 같은 값이 표기에 따라 두 가명을 받는다.
    유출은 아니지만 모델이 두 값으로 읽는다.
    """
    session = Session()

    mask("연락처 010-9921-3348-0000-0002 입니다", session)
    mask("연락처 010-9921-3348-0000-0002 입니다", session)
