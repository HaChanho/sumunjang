...
def test_범주에_잡히지_않는_보이지_않는_문자도_막는다():
    """눈에 보이지 않는 것과 유니코드가 분류하는 방식은 일치하지 않는다.

    U+3164 한글 채움 문자는 범주가 Lo(글자)인데 화면에는 아무것도 그리지 않고,
    한국에서 공백 닉네임용으로 널리 쓰인다. 범주로 한 겹, 목록으로 한 겹 막는다.
    """
    for 이름, 문자 in (
        ("U+3164 한글 채움", "ㅤ"),
        ("U+115F 초성 채움", "ᅟ"),
