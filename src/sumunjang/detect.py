...
_RULES = (
    ...
    (
        "RRN",
        _anchored("주민등록번호|주민번호|주민|rrn|jumin", r"(?<!\d)\d{13}(?!\d)"),
        # 검증기에 넘기기 전에 전각을 반각으로 맞춘다. 생년월일 관문이 자릿값을
        # ASCII 문자와 비교하므로 전각이 그대로 들어가면 조용히 빠져나간다.
        lambda matched: _rrn_birthdate_ok(_digits(matched)),
    ),
    ...
    (
        "BRN",
        _anchored("사업자등록번호|사업자번호|사업자|brn", r"(?<!\d)\d{10}(?!\d)"),
        lambda matched: _brn_checksum_ok(_digits(matched)),
    ),
)
