...
보이지 않는 문자는 **목록이 아니라 유니코드 범주로** 막습니다. 목록으로 관리하다
계속 뚫렸습니다 — U+200B를 막으면 U+2063으로, 그것을 막으면 U+00AD·U+2066·U+034F로
우회합니다. 목록은 언제나 공격자보다 늦습니다. 서식 제어 문자(Cf)와 폭 0 결합
문자(Mn)를 통째로 걷어내면 그 우회 수단 자체가 닫힙니다.
...
세션은 자기가 가린 값을 알고 있습니다. 규칙 탐지에 더해 **세션이 아는 값의 정확
일치를 함께 가려** 고리를 닫습니다. 앵커가 없어도, 코드블록 안이어도, 모델이 설명
대상으로 언급해도 마찬가지입니다.
...
예외는 **딱 둘**입니다 — 추론 서명과 base64로 실린 첨부의 알맹이. 한때 주소와
식별자도 예외였는데 되돌렸습니다. **예외 칸 자체가 우회 통로**였고(식별자에
개인정보를 넣으면 그대로 나갔습니다), `id`는 가리고 `tool_use_id`는 안 가려
오히려 짝이 어긋났습니다. 전부 가리면 같은 값이 같은 가명을 받으므로 짝이 맞고
통로도 사라집니다.

`data:`가 언제나 base64인 것도 아닙니다 — `data:image/svg+xml,<svg>…</svg>`는
평문이라 가립니다. `;base64,`가 붙은 것만 알맹이로 봅니다.
...
| `/v1/chat/completions` | 마스킹해서 전달 (OpenAI 호환) |
...
- **첨부 파일 안은 보지 않습니다.** base64로 실린 이미지·문서의 내용은 해독하지 않으므로,
  ...
  토큰뿐입니다 — 훼손되면 요청 자체가 거부되는 값들입니다. 주소(URL)는 한때 예외로
  뒀다가 되돌렸습니다. 가리면 첨부가 깨지지만 **깨지는 것은 눈에 보이고 유출은
  보이지 않기** 때문입니다.
...
- **OpenAI 호환 경로는 실제 서비스 왕복으로 검증하지 않았습니다.** 저장소에 포함된
  에코 서버로만 시험했습니다. Anthropic 경로는 Claude Code 로 실왕복을 마쳤습니다.
