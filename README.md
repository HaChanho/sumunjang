...
표기가 흐릿할수록 조건을 늘리는 원칙을 일관되게 씁니다.
...
대가도 함께 적습니다. 하이픈 없이 붙여 쓴 2020년 10월 이후 발급분과, 구분자 없는
국내전용 카드는 **놓칩니다.** `goldenset-gaps/` 에 정답으로 박아 두고 점수로 공표합니다.
...
| `goldenset-gaps/` | **못 잡는다고 선언한 것들.** 낮은 것이 정상 | 재현율 0.235 |

`goldenset-hard/` 는 도입 당시 **0.684 / 0.765** 였습니다. 그 자리가 드러낸 결함 넷을
고쳐 지금 수치가 됐습니다. `goldenset-gaps/` 의 항목이 `hard` 로 옮겨가는 것이 곧 개선
기록입니다.
...
- **이름·주소는 못 잡습니다.** 문맥 없이는 판별할 수 없습니다. `goldenset-gaps/G2` 에
  정답으로 박아 두고 0점으로 공표합니다. 문맥 앵커 규칙이 다음 단계입니다.
- **계좌번호·여권번호·운전면허번호도 못 잡습니다.** 검증식이 없거나(계좌) 공개되지
  않았습니다(면허 검사번호). 형태만으로 잡으면 오탐이 터지므로 잡지 않기로 했습니다.
- **관문을 좁힌 대가가 있습니다.** 하이픈 없이 붙여 쓴 2020년 10월 이후 주민등록번호와,
  구분자 없는 국내전용 카드는 놓칩니다. 위 표의 `gaps` 셋에 들어 있습니다.
